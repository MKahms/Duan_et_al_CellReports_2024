...
Function BoxSmoothing (list_name, points)//BoxSmoothing(intens_index_norm,45)
...
for (i=1;i<numpnts(list_name);i+=1)
	...
	Wave current=$list_name[i]
   Duplicate/O current,current_smth;DelayUpdate
   Smooth/EVEN/B points,current_smth;DelayUpdate
   current-=current_smth
...
Function GluSnFR (list_name, sdev1, sdev2)

Wave/T list_name
variable sdev1, sdev2

variable j,i,k, SD, success, amplitude, peak, peak1, peak2, base, delta, delta1, delta2, a,b,d, number_minis, amplitude_minis
...
Make/N=(numpnts(list_name)-1)/O/T ROI
Make/N=(numpnts(list_name)-1)/O/D ROI_success
Make/N=(numpnts(list_name)-1)/O/D ROI_amplitude
Make/N=3000/O/D amplitudes_all
Make/N=3000/O/D timepoints
Make/N=3000/O/D Amplitudes_minis_all
Make/N=(numpnts(list_name)-1)/O/D mini_number
Make/N=(numpnts(list_name)-1)/O/D mini_amplitude
...
for (i=1;i<numpnts(list_name);i+=1)
    ...
    Wave current=$list_name[i]
    WaveStats/Q/R=[sdev1,sdev2] current
    SD=v_sdev
    success=0
    amplitude=0
		...
		k=99
    		...
    		base= (current[k-3]+current[k-4]+current[k-5])/3
    		delta= peak-base
    	
    		if (delta > (4*SD))
    		success+=1
    		amplitude+=delta
    		amplitudes_all[j]=delta
    		j+=1
...
ROI[i-1]=list_name[i]
ROI_success[i-1]=success
ROI_amplitude[i-1]=amplitude




	
    	for (k=sdev1;k<sdev2;k+=1)
    	
    		a=(current[k]+current[k+1]+current[k+2]+current[k+3]+current[k+4]+current[k+5]+current[k+6]+current[k+7]+current[k+8]+current[k+9])/10
    		b=current[k+11]
    		...
    		if (delta > (4*SD))
    		number_minis+=1
    		amplitude_minis+=delta
    		timepoints[d]=k+11
    		amplitudes_minis_all[d]=delta
...
amplitude_minis/=number_minis


mini_number[i-1]=number_minis
mini_amplitude[i-1]=amplitude_minis
number_minis=0
amplitude_minis=0
...
DeletePoints d,1500, Amplitudes_minis_all

DeletePoints d,1500, timepoints

WaveStats/Q ROI_success

DeletePoints v_sum,1500, amplitudes_all



edit ROI, ROI_success, ROI_amplitude, mini_number, mini_amplitude, amplitudes_all, amplitudes_minis_all, timepoints
print SD

for (i=0;i<numpnts(timepoints);i+=1)

	if (timepoints[i]==0)
		 	DeletePoints i,numpnts(timepoints)-(i-1), timepoints
...
for (i=0;i<numpnts(amplitudes_all);i+=1)

	if (amplitudes_all[i]==0)
		DeletePoints i,numpnts(amplitudes_all)-(i-1), amplitudes_all
...
for (i=0;i<numpnts(amplitudes_minis_all);i+=1)

	if (amplitudes_minis_all[i]==0)
		DeletePoints i,numpnts(amplitudes_minis_all)-(i-1), amplitudes_minis_all
